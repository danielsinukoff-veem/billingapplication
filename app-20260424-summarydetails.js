...
} from "./shared-backend-20260424-linkdownloads.js";
...
const UNTRUSTED_DETAIL_SOURCES = new Set([
  "stampli_usd_abroad_revenue",
  "stampli_usd_abroad_reversal"
]);
...
const INVOICE_METADATA_BILLING_KEYS = new Set([
  "invoiceEntity",
  "partnerLegalName",
...
function getTableConfigs() {
  return {
    ...
    lrs: [{ key: "period", label: "Period", w: 90 }, pCol(), { key: "netRevenue", label: "Net Rev", type: "number", w: 110 }, { key: "partnerRevenueShare", label: "Partner Share (Payable)", type: "number", w: 140 }, { key: "revenueOwed", label: "Revenue Owed (Receivable)", type: "number", w: 150 }, { key: "monthlyMinimumRevenue", label: "Monthly Min Fee", type: "number", w: 130 }],
...
function evaluatePayableTrackingStatus({ amountDue, amountPaid, invoiceDate }) {
  ...
  if (invoiceDate) {
    return { kind: "open", label: "Payables", tone: "credit" };
  }
  return { kind: "credit", label: "Credit to Partner", tone: "credit" };
}
...
function summarizePartnerOutstanding(partner, { period = "" } = {}) {
  const summaryPeriod = normalizeMonthKey(period);
  const entries = getPartnerInvoiceTrackingEntries(partner)
    .filter((entry) => !summaryPeriod || entry.period === summaryPeriod);
  const today = todayIsoDate();
  let owesUs = 0;
  let weOwePartner = 0;
  let partialCount = 0;
  ...
  if (maxPastDueDays > 0) {
    ...
    status = { tone: "credit", label: "Payables" };
  } else {
...
function getPartnerOwesLabel(partner, { plural = false } = {}) {
  return plural ? "Receivables" : "Receivables";
}
...
function renderBillingSummaryInvoiceDetails(summary) {
  const entries = (summary.entries || []).filter((entry) => entry.amountDue > 0 || entry.record);
  if (!entries.length) {
    return `<div class="empty-state compact">No invoice rows match this summary filter.</div>`;
  }
  return `
    <div class="billing-summary-detail-card">
      <table class="billing-summary-detail-table">
        ...
        <tbody>
          ${entries.map((entry) => `
            <tr>
              <td>${html(formatPeriodLabel(entry.period))}</td>
              <td>${entry.kind === "payable" ? "Payables" : "Receivables"}</td>
              <td>${renderInvoiceStatusTag(entry.status)}</td>
              <td>${entry.invoiceDate ? html(formatIsoDate(entry.invoiceDate)) : "—"}</td>
              <td>${entry.dueDate ? html(formatIsoDate(entry.dueDate)) : "—"}</td>
              <td class="mono align-right">${fmt(entry.amountDue)}</td>
              <td class="mono align-right">${entry.amountPaid > 0 ? fmt(entry.amountPaid) : "—"}</td>
              <td class="mono align-right">${fmt(entry.balance)}</td>
            </tr>
          `).join("")}
        </tbody>
      </table>
...
function buildInvoicePdfDocument(doc, { autoPrint = true } = {}) {
  ...
  const summaryLabel = isReceivable ? "Balance Due" : "Payables";
  const totalLabel = isReceivable ? "Total" : "Payables Total";
...
function buildInvoicePdfPageContent(doc) {
  ...
  [
    ...
    [isReceivable ? "Balance Due" : "Payables", formatUsdAmount(doc.amountDue)],
    [isReceivable ? "Total" : "Payables Total", formatUsdAmount(doc.amountDue)]
...
function buildInvoiceDocument(inv, kind) {
  ...
  return {
    ...
    amountLabel: kind === "receivable" ? getPartnerOwesLabel(inv.partner) : "Payables",
...
const DETAIL_SUMMARY_NUMERIC_FIELDS = new Set([
  "avgTxnSize",
  "customerRevenue",
  "directInvoiceAmount",
  "directInvoiceRate",
...
function coerceDetailSummaryNumber(value) {
  if (value === null || value === undefined || value === "") return 0;
  const parsed = Number(String(value).replace(/[$,]/g, ""));
  return Number.isFinite(parsed) ? parsed : 0;
}

function normalizeDetailSummaryTxnRow(row) {
  if (String(row?.sourceSection || "") !== "ltxn") return null;
  const partner = String(row.partner || "").trim();
  const period = String(row.period || "").trim();
  if (!partner || !period) return null;
  const output = {
    id: row.id || uid(),
    partner,
    period,
    txnType: String(row.txnType || "").trim(),
    speedFlag: String(row.speedFlag || "").trim(),
    processingMethod: String(row.processingMethod || "").trim(),
    payerFunding: String(row.payerFunding || "").trim(),
    payeeFunding: String(row.payeeFunding || "").trim(),
    payerCcy: String(row.payerCcy || "").trim(),
    payeeCcy: String(row.payeeCcy || "").trim(),
    payerCountry: String(row.payerCountry || "").trim(),
    payeeCountry: String(row.payeeCountry || "").trim()
  };
  DETAIL_SUMMARY_NUMERIC_FIELDS.forEach((field) => {
    output[field] = coerceDetailSummaryNumber(row[field]);
  });
  ["directInvoiceSource", "revenueBasis"].forEach((field) => {
    if (row[field] !== undefined && row[field] !== null && row[field] !== "") {
      output[field] = String(row[field]).trim();
    }
  });
  return output;
}

async function hydrateMissingInvoiceSummaryRowsForRange(partner, startPeriod, endPeriod) {
  const periods = enumeratePeriods(startPeriod, endPeriod);
  let hydratedCount = 0;
  for (const period of periods) {
    ...
    const summaryRows = detailRows
      ...
      .filter((row) => row && norm(row.partner) === norm(partner) && row.period === period && !isUntrustedDirectInvoiceRow(row));
    if (!summaryRows.length) continue;
    state.ltxn = [
      ...(state.ltxn || []).filter((row) => !(norm(row.partner) === norm(partner) && row.period === period && !isUntrustedDirectInvoiceRow(row))),
      ...summaryRows
    ];
    hydratedCount += summaryRows.length;
  }
  if (hydratedCount) monthlyInvoiceCache.clear();
...
function getStampliFxPartnerMarkupDirection(group, summaryRow = {}) {
  const textValue = [
    group?.label,
    ...(group?.lines || []).map((line) => line.desc),
    summaryRow.txnType
  ].filter(Boolean).join(" ").toLowerCase();
  if (!textValue.includes("fx partner markup")) return "";
...
function matchesStampliFxPartnerMarkupDetail(detailRow, summaryRow, group) {
  ...
  const detailDirection = detailRow.stampliFxDirection
...
async function buildInvoiceArtifactPayload(inv = state.inv, { trigger = "generate_invoice" } = {}) {
  ...
  const documents = buildInvoiceDocuments(inv).map((doc) => ({
    ...
    pdfFileName: `${bundleKey}-${doc.kind === "receivable" ? "ar-invoice" : "ap-invoice"}.pdf`,
    amountDue: doc.amountDue,
    pdfHtml: buildInvoicePdfDocument(doc, { autoPrint: false }),
    pdfBytes: buildInvoicePdfBytes(doc)
  }));
...
function applyInvoiceResult(invoice) {
  ...
  if ((invoice.periodStart || invoice.period) === (invoice.periodEnd || invoice.period)) {
    state.financeBatchPeriod = invoice.periodStart || invoice.period || state.financeBatchPeriod;
  }
  render();
}
...
function calculateLocalInvoiceForPeriod(partner, period, options = {}) {
  ...
  return {
    ...
    pay,
    net,
    dir: net >= 0 ? "Receivables" : "Payables"
  };
}
...
async function calculateInvoice() {
  ...
  try {
    ...
    await hydrateMissingInvoiceSummaryRowsForRange(state.sp, state.perStart, state.perEnd);
...
function renderBillingSummary() {
  ...
  const summaryPeriod = normalizeMonthKey(state.billingSummaryPeriod || "");
  const summaries = getPartnerOptions({ includeArchived: false, includeArchivedTag: false })
    .map((option) => summarizePartnerOutstanding(option.value || option, { period: summaryPeriod }))
    ...
    .filter((summary) => {
      ...
      if (summaryPeriod) return summary.entryCount > 0;
  ...
  return renderSection({
    ...
    content: `
      <div class="stack">
        <div class="billing-summary-toolbar">
          <div class="billing-summary-filters">
            <label class="field">
            ...
            <label class="field">
              ${renderLabelWithInfo("Month Filter", "Optional. Choose a month to show receivables and payables for that invoice month only. Partner and month filters can be used separately or together.")}
              <input class="input" type="month" data-bind="billingSummaryPeriod" value="${html(summaryPeriod)}">
            </label>
            <div class="field">
              <span class="label">Filters</span>
              <button class="button ghost small" data-action="clear-billing-summary-filters">Clear Summary Filters</button>
            </div>
          </div>
          <div class="billing-summary-kpis">
            <div class="kpi-card">
            ...
            <div class="kpi-card ${net >= 0 ? "info" : "warning"}">
              ...
              <span>${net >= 0 ? "Net Receivables" : "Net Payables"}</span>
            </div>
          </div>
        </div>
        <div class="table-wrap">
          <table class="data-table billing-summary-table">
            ...
            <tbody>
              ${!summaries.length ? `<tr><td colspan="5" class="empty-state">No matching partners in the summary filter.</td></tr>` : !rowsExpanded ? "" : summaries.map((summary) => {
                const detailKey = sectionKey(`billing-summary-partner:${summary.partner}`);
                const detailOpen = isSectionOpen(detailKey, false);
                return `
                  <tr
                    class="billing-summary-partner-row"
                    data-action="toggle-billing-summary-partner"
                    data-key="${html(detailKey)}"
                    aria-expanded="${detailOpen ? "true" : "false"}"
                    title="Show invoice list for ${html(summary.partner)}"
                  >
                    <td><span class="toggle-indicator">${detailOpen ? "▾" : "▸"}</span>${html(summary.partner)}</td>
                  ...
                  ${detailOpen ? `
                    <tr class="billing-summary-detail-row">
                      <td colspan="5">${renderBillingSummaryInvoiceDetails(summary)}</td>
                    </tr>
                  ` : ""}
                `;
              }).join("")}
  ...
  return `
    <div class="stack">
      <div class="field-grid billing-field-grid">
        ...
        <label class="field billing-span-2">
          ${renderLabelWithInfo("Invoice Bill-To Name", "Legal or billing name that appears in the Bill To block on generated invoices.")}
          <input class="input" type="text" value="${html(config.partnerLegalName || partner)}" placeholder="${html(partner)}" data-action="update-partner-billing" data-partner="${html(partner)}" data-key="partnerLegalName">
        </label>
        <label class="field field-wide billing-span-2">
          ${renderLabelWithInfo("Invoice Bill-To Address", "Partner billing address that appears under the partner name on generated invoices. Put each address line on a separate line.")}
          <textarea class="textarea textarea-compact" data-action="update-partner-billing" data-partner="${html(partner)}" data-key="partnerBillingAddress" placeholder="Street address&#10;City, State ZIP&#10;Country">${html(config.partnerBillingAddress || "")}</textarea>
        </label>
        <label class="field">
        ...
        <table class="data-table invoice-tracker-table">
          ...
          <tbody>
            ${!entries.length ? `<tr><td colspan="8" class="empty-state">No invoice periods available yet. Import activity first, then invoice tracking will appear here.</td></tr>` : !rowsExpanded ? "" : entries.map((entry) => {
              ...
              const kindLabel = entry.kind === "payable" ? "Payables" : getPartnerOwesLabel(partner);
              return `
                <tr>
                  <td>
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          <tfoot>
            ...
            <tr><td colspan="2">${html(doc.kind === "receivable" ? "BALANCE DUE" : "PAYABLES")}</td><td colspan="2" class="align-right mono">${fmt(doc.amountDue)}</td></tr>
          </tfoot>
        </table>
      </div>
    </div>
...
function renderInvoiceTab() {
  ...
  ` : "";
  const showDeliveryPanel = state.inv && (
    state.privateInvoiceLinkStatus !== "idle"
    || !!state.privateInvoiceLinkResult
    || !!state.privateInvoiceLinkError
  );
  ...
  const financeBatchPanel = renderSection({
    key: "finance-batch-downloads",
    title: "Finance Batch Downloads",
    ...
    content: `
      ...
      <div class="field-grid" style="margin-top:16px">
        <label class="field">
          ...
          <input class="input" type="month" data-bind="financeBatchPeriod" value="${html(state.financeBatchPeriod || state.inv?.periodStart || state.perStart || LOOKER_IMPORT_PERIOD)}">
        </label>
        <div class="field">
      ...
      <div class="card">
        ...
        <div class="field-grid" style="margin-top:16px">
          <label class="field">
            ...
            <select class="select" data-bind="sp">
              ${renderOptions([{ value: "", label: "Select partner..." }, ...getPartnerOptions({ includeArchived: false })], state.sp)}
            </select>
          </label>
          <label class="field">
...
root.addEventListener("click", (event) => {
  ...
  if (action === "clear-billing-summary-filters") {
    state.billingSummaryPartner = "";
    state.billingSummaryPeriod = "";
    render();
    return;
...
root.addEventListener("change", (event) => {
  ...
  if (target.matches("[data-action='update-partner-billing']")) {
    ...
    if (state.inv && norm(state.inv.partner) === norm(partner) && INVOICE_METADATA_BILLING_KEYS.has(key)) {
      state.invoiceArtifactStatus = "idle";
      state.invoiceArtifactError = "";
      state.invoiceArtifactRecord = null;
      state.privateInvoiceLinkStatus = "idle";
